...
쿼리에는 익숙하지 않았던 분들을 위한 초보자용 강의입니다. 일반적인 데이터베이스 강의라면 데이터베이스 개론부터 여러가지 이론 내용으로 시작하다 보니 처음 보는 생소한 용어들 때문에 진입장벽이 느껴지는 경우가 있습니다. 하지만 서버 관리를 하거나 관련 전문 개발자로 커리어를 삼는게 아닌 데이터 분석만을 목적을 두고 배울때 필요한 기능을 배우는 것은 어렵지 않습니다.
본 포스팅 에서는 데이터를 잘 다루는 데에 필요한 부분만을 가르치고자 합니다. 

여러 분의 이해도를 높이기 위해 실전 데이터를 예제로 가져왔습니다.

### 데이터베이스

데이터베이스는 데이터의 집합입니다. 많은 사용자가 각자의 PC나 공간에 동일한 데이터를 저장하는 것은 공간과 관리 측면에서 비 효율적 입니다.데이터를 중복 없이 한 곳에 모아두면 관리 효율성이 증대되고 처리 속도도 크게 향상됩니다. 

초창기에는 데이터베이스를 단순히 데이터의 집합으로 봤습니다. 그런데 시간이 지나면서 데이터베이스 내의 데이터를 보다 효과적으로 관리하기 위한 다양한 기능과 규칙이 도입되어 통합됩니다. 이는 데이터의 검색 속도를 높이고, 갱신 및 관리를 용이하게 하는데 도움을 주게됩니다. 우리는 이를 데이터베이스 관리 시스템이라고(DBMS) 부릅니다. 이제는 데이터의 단순 집합과 관리 시스템을 포함하여 데이터베이스라고 부르게 됩니다.

### 데이터베이스 관리 프로그램(DBMS)

데이터를 한 곳에 모아서 다수의 사용자가 동시에 이용하게 되면 여러가지 예상치 못한 문제가 발생할 수 있습니다.

예를 들어, 많은 사용자가 동시에 접속할 때, 누구의 요청을 먼저 처리할 것인지 결정해야 합니다. 단순히 요청 순서대로 처리할 수도 있지만, 사용자의 중요도에 따라 우선순위를 다르게 설정할 수도 있습니다. 월급날에 인사팀장이 데이터 조회나 업데이트를 요청하면, 다른 직원이 자신의 월급을 조회하는 것보다 우선순위가 높아야 합니다. 이 경우, 일반 직원의 요청을 잠시 대기시키고 인사팀장의 요청을 먼저 처리해야 합니다.

혹은 한 사용자가 데이터를 조회하는 도중에 다른 사용자가 그 데이터를 갱신하거나 삭제하는 경우가 있습니다. 이때 데이터베이스 시스템은 요청 시점의 데이터를 보여줄 것인지, 아니면 갱신된 최신 데이터를 보여줄 것인지 결정해야 합니다.

 정전과 같은 예상치 못한 상황에서 데이터가 손상될 경우, 데이터를 복구해야 합니다. 예를 들어, 데이터를 업데이트 중에 정전이 발생하여 데이터 업데이트 중간에 다운된다면, 다시 이전 상태로 복구해야 합니다. 은행에서 계좌로 송금되던 돈이 어떤 이유로 송금이 중간에 중단되었을때 송금이 되기 전 상태로 되돌려야 한다는 것과 같은 맥락입니다.
...
SQL은 관계형 데이터베이스(RDBMS)를 제어하기 위해 사용되는 언어로, 데이터의 검색, 관리, 생성, 수정, 그리고 접근 조정 관리(예: 사용자별로 역할을 다르게 설정하는 작업)를 위해 설계되었습니다. 다양한 관계형 데이터베이스가 존재하지만, 1987년에 SQL 표준이 결정되면서 서로 다른 관계형 데이터베이스들도 같은 표준을 따르게 되었습니다. 그래서 하나의 RDBMS를 잘 배우면, 다른 관계형 데이터베이스들도 어렵지 않게 사용할 수 있습니다.

본 수업에서는 Microsoft SQL Server를 선택했습니다. 

그 이유는 다음과 같습니다.

**왜 Microsoft SQL Server 인가?**

Microsoft SQL Server, MySQL, Oracle SQL 등은 모두 SQL 기반으로 되어 있어, 거의 같은 문법으로 프로그래밍 합니다. 무엇을 사용할지는 개인이나 회사의 선택입니다. 저는 초창기 데이터 인프라를 구축할 때 Oracle을 사용했습니다. 그러다 해외 데이터 벤더와 계약할 때, 벤더가 데이터를 SQL Server로만 제공한다고 해서 둘을 혼용해서 사용하게 되었습니다. 이후에는 SQL Server를 더 많이 사용하게 되었습니다. 제가 익숙하다는 이유도 있지만, 사실 본 강의에서 SQL Server를 선택한 것은 초보자나 윈도우 환경에서 작업하는 대부분의 사람들에 확실히 유리한점이 있기 때문입니다. (Mac OS, Linux에서도 사용 가능)

첫번째로, SQL Server는 현업에서 가장 많이 사용하는 프로그램인 Excel과의 연동이 추가 환경 설정 없이 자동으로 됩니다. 두번째로, 인공지능(AI) 시대에 유용한 R이나 파이썬 같은 언어와의 연계가 SQL Server 2016년부터 내장 기능으로 포함되었습니다. 즉, R이나 파이썬에서 SQL과 연동하는 것이 아니라, SQL에서 R이나 파이썬 코드를 실행한다는 것입니다. 이 기능이 추가된 이후, 자동화와 유지보수가 훨씬 간편해졌습니다.

MS SQL Server는 설치 및 유지 관리가 편리하고, 질의 도구, 데이터베이스 관리 도구, XML이나 웹 응용 도구들의 사용이 매우 편리합니다. Oracle과 MySQL에 비해 데이터베이스 관리 차원에서의 난이도가 낮은 편입니다. 그래서 처음 접하는 초보자들이 사용하기에 적합하다고 판단했습니다.

저는 과거 Oracle을 먼저 접했고, SQL Server를 이후에 사용해보고, 현재는 MySQL도 종종 사용하는데, 세 가지를 사용하는데 크게 불편함을 느끼지 않고 있습니다. 여러분도 Microsoft SQL Server를 배우면, 나중에 Oracle이나 MySQL도 금방 익힐 수 있다는 점을 강조드립니다.
...
테이블에는 특정 정보를 담은 파일들이 주제별로 정리되어 있으며, 많은 경우 어떤 규칙이 있는 이름을 붙여 나중에 쉽게 찾을 수 있도록 정리되어 있습니다. 테이블의 이름은 언제나 고유해야 하며, 데이터베이스의 다른 테이블과 같은 이름을 사용해서는 안됩니다. 테이블은 열(Column)과 행(Row)으로 구성되어 있습니다. 열은 필드(Field)라고도 부르고 행은 레코드(record)라고 부르기도 합니다. 열에 존재하는 데이터는 같은 속성을 가진다고 하여 필드, 행에 존재하는 데이터는 여러 속성을 가진 한 개체의 모임이라고 해서 레코드 라고 부릅니다.
...
GROUP BY와 병행해서 사용하는 집계함수에 대하여 알아봤는데요. 이번에는 행과행의 관계를 설명하는 윈도우 함수들에 대하여 알아보겠습니다. 윈도우 함수를 사용하면 해당 행 데이터의 순위,위치 등을 조작할 수 있습니다. 윈도우 함수는 PARTITION BY 구문을 통해 분할 대상을 기준으로 적용이 가능합니다. GROUP BY에서 사용하던 집계함수인 sum, max, min 등과 같은 함수도 PARTITION BY 로 범위를 지정하면 윈도우 함수처럼 사용할 수 있습니다.
...
금융 시계열 데이터에는 현재 값과 이전, 이후 값을 비교하는 코드를 짤 때가 많습니다. 그런 예시를 한번 보겠습니다. 여기서는 ID : 40853 삼성전자 수정주가 기준 종가, 어제종가, 내일종가를 한꺼번에 보여주는 코드를 짜보겠습니다. 
...
---------------------------------------------------
2023-11-21 00:00:00.000	72800	72700	NULL
2023-11-20 00:00:00.000	72700	72500	72800
2023-11-17 00:00:00.000	72499	72800	72700
2023-11-16 00:00:00.000	72800	72200	72500
2023-11-15 00:00:00.000	72200	70800	72800
2023-11-14 00:00:00.000	70799	70400	72200
...
```
...
SELECT	MarketDate
	...
	,	LEAD(adj,1) OVER (ORDER BY MarketDate)/adj -1.0 AS 내일수익률
FROM	adjprc
WHERE	Infocode = 40853
ORDER	BY	MarketDate desc    
```
...
------------------------------------------------------
2023-11-21 00:00:00.000	72800	0.0013754677942337		NULL
2023-11-20 00:00:00.000	72700	0.00275884520655056		0.0013754677942337
2023-11-17 00:00:00.000	72500	-0.00412105433681742	0.00275884520655056
2023-11-16 00:00:00.000	72800	0.00831022733685316		-0.00412105433681742
2023-11-15 00:00:00.000	72200	0.019774143756748		0.00831022733685316
2023-11-14 00:00:00.000	70800	0.00568189593876967		0.019774143756748
2023-11-13 00:00:00.000	70400	-0.00141871942378369	0.00568189593876967
...
```
...
2023-11-22 00:00:00.000	2323	0.0104892674108843
2023-11-22 00:00:00.000	151515	0.0190173994164511
2023-11-22 00:00:00.000	210750	-0.00996862543894417
2023-11-22 00:00:00.000	169686	-0.00840681572950652
2023-11-22 00:00:00.000	258836	0.0212070109110531
2023-11-22 00:00:00.000	2209	0.00396663701821121
2023-11-22 00:00:00.000	151500	0.0177511519549178
2023-11-22 00:00:00.000	804		0.0174858411614005
2023-11-22 00:00:00.000	10241	0.000982840148579855
2023-11-22 00:00:00.000	286851	-0.020167522560702
2023-11-22 00:00:00.000	263294	0.00894401917893317
...
```
...
정말 많이 사용하는 구간을 지정하여 함수를 적용하는데 필요한 문법입니다.
...
여기서 N PRECEDING을 하면 N개 전 데이터를 가져오고 N FORWARDING 하면 N개 뒤의 데이터를 가져옵니다. BETWEEN N PRECEDING[FORWARDING] AND N PRECEDING[FORWARDING] 은 순서를 감안해 구역을 한번 더 나눈다고 이해하시면 됩니다. 투자모델 개발때 FORWARDING 문법은 사용해본 경험이 없을 정도로 잘 쓰이지 않습니다.
...
연결하고자 하는 두 테이블은 서로 같은 유형의 열을 가지고 있어야 합니다.  조인할 테이블을 JOIN으로 지정하고 그 기준이 되는 열(필드)을 ON 으로 지정하여 연결합니다. JOIN은 여러 열을 기준으로 할 수도 있습니다.
...
다음은 2023년10월31일 주식들의 수정주가를 알고자 하는 코드입니다.
...
--------------------------------------------
S-1										54600
ZHEJIANG HUAHAI PHARM. 'A'				16.17
SINDOH									31950
TREND MICRO								5642
E ON N									11.22
CNPC CAPITAL 'A'						5.93
KIKKOMAN								8521
SINOPEC OILFIELD SERVICE 'A'			1.94
FINAXA DEAD - 21/12/05					NULL
.
.
```

adj가 NULL로 되어있는 것은 2023년 10월 31일에 가격정보가 존재하지 않는 종목입니다. 종목명을 보면 상장폐지가 되었거나 합병이 되어서 DEAD 상태라는 정보가 보입니다.

RIGHT JOIN 구문은 LEFT와 동일한 의미에 시작 테이블 위치만 바뀐 것입니다.  실무에서는 자주 사용하지 않습니다. 저도 실전에서는 RIGHT JOIN을 사용할바에 테이블 위치를 바꿔 LEFT를 사용하는 경우가 많았습니다. 그래서 예제를 생략하겠습니다.

FULL OUTER JOIN

모든 조합을 다 감안하는 방법입니다.
두 테이블로 나올 수 있는 모든 경우의 수로 연결한 다음 조건을 찾아 나가는 형식입니다. 
LEFT JOIN, RIGHT JOIN의 결과의 합집합으로 불 수 있습니다. 실무에서는 잘 사용하지 않았습니다. 예제는 생략하겠습니다.

**JOIN 중첩(NESTED LOOP JOIN)**

JOIN은 계속 중첩해서 사용할 수 있습니다. NESTED LOOP JOIN은 2개 이상의 테이블에서 하나의 집합을 기준으로 순차적으로 행과 행을 계속해서 결합하여 원하는 결과를 조합하는 조인 방식입니다. 

A라는 테이블을 B라는 테이블과 연결하고 그 결과에 C라는 테이블을 또 연결하고 또 D라는 테이블을 그 결과와 연결하는등의 작업을 많이 합니다. 

문법 자체는 특별히 달라지는게 없습니다. JOIN을 연속해서 계속 사용하기만 하면 됩니다.
...
다음 쿼리는 2023년 10월 31일 기준 기업들을 시가총액으로 정보를 보여줍니다. 내림차순 정렬을하고 해당 기업의 이름과 어떤 산업에 소속되어 있는지를 같이 보여줍니다.
...
Join 구문은 관계형데이터베이스가 주류가 되도록 만든 가장 큰 이유입니다. 데이터의 성격별로 테이블을 따로 구분하고 합칠때는 JOIN을 사용하여 원하는 형태로 보여주는 방식이 RDB의 핵심입니다. RDB에서는 데이터 관리도 쉽고 공간도 적게 사용하며 그래서 검색도 빠르게 되는등 장점이 많고 이 장점을 사용자들이 동의하여 주류가 된 것입니다.
...
SELECT 쿼리문의 결과는 테이블입니다. 테이블을 구분하면 스칼라 유형의 단일 값만을 가지는 테이블, 하나의 열 밖에 없는 단일 열 테이블, 여러개의 열을 가지는 다중 열 테이블로 나눌수 있습니다. 공식적으로 세분화 하면 다음과 같이 분류됩니다. 
...
------------------------------------
2023-07-31 00:00:00.000	72990		195.9
```

애플의 주가가 최대였던 시점은 2023년 7월 31일인 것을 확인해 볼수 있었습니다.
...
-----------------------------------------------------
2023-11-21 00:00:00.000	72990	190.6	0.972986267704808
2023-11-20 00:00:00.000	72990	191.5	0.977120408856379
2023-11-17 00:00:00.000	72990	189.7	0.968137554233967
2023-11-16 00:00:00.000	72990	189.7	0.968239867652348
2023-11-15 00:00:00.000	72990	188		0.959563294506261
2023-11-14 00:00:00.000	72990	187.4	0.956654228584697
...
```
...
시가총액이 2023년 10월 31일 기준 1조달러를 넘는 기업의 주식들을 기업명으로 반환해 보겠습니다.
```sql
SELECT	DSQTName
FROM	secinfo
WHERE	InfoCode IN (SELECT Infocode FROM factors where date_='20231031' and mktcapusd>=1000000.0)
```

서브쿼리인 (SELECT Infocode FROM factors where date_='20231031' and mktcapusd>=1000000.0)의 반환결과는 Infocode로 구성된 단일열 테이블입니다.
secinfo에서 Infocode가 이 단일열 테이블에 존재하는 주식들의 이름을 SELECT에서 보여주도록 합니다.

*다중 열 테이블 형태(행렬) 서브쿼리*

주로 FROM절에서 사용하는 다중 열 테이블입니다. 사실 FROM 절에는 모든 종류의 서브쿼리가 다 위치할 수 있으나 원래 테이블이 위치해야 하는 곳이다 보니 다중 열 테이블을 반환하는 서브쿼리가 거의 대부분을 차지합니다. 서브쿼리의 반환결과가 기존의 테이블과 같으므로 FROM절에 서브쿼리를 위치시키면 됩니다. 다만 서브쿼리 결과에는 이름이 없으니 이를 지정해 줘야 합니다.
...
다음은 한국 기업들을(2023년 10월 31일 기준) 시가총액 순서로 정렬해 보여주되 기업명을 포함하라는 쿼리입니다. 시가총액이 50조원 이상인 기업으로 한정지었습니다.
...
(
		...
		join	secinfo i
			on	f.infocode = i.InfoCode
		WHERE	F.date_ = '20231031' AND F.region='KR'
) M
...
DsQtName			mktcap
--------------------------------
SAMSUNG ELECTRONICS	399378399.7
LG ENERGY SOLUTION	90207000
SK HYNIX			84666660.5
SAMSUNG BIOLOGICS	50462408.5
...
```
...
---------------------------------------------------------------------------------------
2023-11-21	40853		374045.1	SAMSUNG ELECTRONICS	Phones & Handheld Devices (NEC)
2023-11-20	40853		371489.8	SAMSUNG ELECTRONICS	Phones & Handheld Devices (NEC)
2023-11-19	40853		370568.2	SAMSUNG ELECTRONICS	Phones & Handheld Devices (NEC)
2023-10-31	40853		296605.9	SAMSUNG ELECTRONICS	Phones & Handheld Devices (NEC)
2023-09-30	40853		334218.3	SAMSUNG ELECTRONICS	Phones & Handheld Devices (NEC)
2023-08-31	40853		335260		SAMSUNG ELECTRONICS	Phones & Handheld Devices (NEC)
2023-07-31	40853		364353.4	SAMSUNG ELECTRONICS	Phones & Handheld Devices (NEC)
...
```
...
date_		infocode	mktcapusd	dsqtname	title
------------------------------------------------------------
2023-11-21	72990	2964976	APPLE	Phones & Smart Phones
2023-11-20	72990	2977574	APPLE	Phones & Smart Phones
2023-11-19	72990	2950201	APPLE	Phones & Smart Phones
2023-10-31	72990	2662352	APPLE	Phones & Smart Phones
...
...
```
...
UPDATE구문은 테이블에 조건을 만족하는 행이 이미 존재하는 경우 해당 행의 특정 열값을 수정하고 싶을때 사용합니다.
...
전형적인 예로 우리나라의 주민등록번호가 있습니다. 주민등록번호의 특징은 한 사람을 하나의 주민등록번호로 특정할수 있다는 것이고 중복되지 않으며 주민등록번호가 없는 성인이 없다는 것입니다.
...
즉 자기 테이블에서는 기본키가 아니지만 다른 테이블의 기본키인 열과 같은 열이 자기 테이블에서 외래키 라는 제약조건으로 설정된 것입니다.
...
A 테이블에는 종목코드와 가격이 있고 B 테이블에는 종목코드와 종목의 이름이 있습니다. 둘을 조인을 하면 

종목이름과 종목의 가격을 보기 좋게 보여줄 수 있습니다.
...
style : CONVERT 함수가 식을 변환할때 그 형태를 지정하는 것입니다. style 은 생략할 수 있습니다.
datetime, float, real, money, smallmoney, xml, binary, char, varbinary, varchar 등에서 사용할수 있습니다.
