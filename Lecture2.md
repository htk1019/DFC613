...
$$
$$
= \frac{D_{t+1}}{P_t} + \frac{P_{t+1} - P_t}{P_t}
$$
...
가격 잉여 : 가격 잉여는 실제로 주식의 가격이 예상된 가격 변동보다 얼마나 많이 변동했는지를 나타내는 지표입니다. 예상된 가격 변동은 주로 회사의 순이익과 배당금에 의해 결정됩니다.

$P_{t+1}$은 변화된 주식가격, $P_t$ 는 매수 시점의 주식가격을 의미합니다. $P_t + NI_{t+1} - D_{t+1}$ 는 주주들이 다음시점 t+1에 기대하는 가격을 의미합니다. 
