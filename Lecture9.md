...
두 가지의 팩터 모델을 살펴 봤습니다. 모든 팩터 모델의 근간은 한 가지 아이디어입니다.
평균 주식 수익률은 팩터 프리미엄과 팩터 노출도의 곱과 같다는 것입니다.
이런 모델들은 과거 데이터를 기반으로 구축되지만 퀀트 운용역은 미래 주식 수익률을 예측하고자 하기 때문에 미래 팩터 프리미엄과 미래 팩터 노출도가 필요합니다.
공식으로 설명하자면 T+1 시점의 주식 수익률을 예측하는 것입니다.
...
펀더멘털 팩터 모델의 간단한 버전을 사용할 경우, 미래 주식 수익률을 예측하기 위해 따로 팩터 익스포저나 팩터 프리미엄의 미래 값을 예측할 필요가 없습니다.

펀더멘털 팩터 모델에서 팩터 익스포저$\beta_{i,t}$는 주식이나 포트폴리오의 팩터에 대한 B/P, E/P 같은 현재의 지표입니다. 이는 이미 현재의 정보를 바탕으로 정해진 값입니다. 따라서 시간 t에서 이 값을 우리는 이미 알고 있습니다. 그리고 그 값으로 $r_{i,T+1}$을 예측합니다. 과거 수익률 회귀분석을 어떻게 했는지 기억해보세요. 오늘 시점의 B/P 같은 팩터 노출도와 이후 수익률을 회귀분석 했었습니다. 즉 우리는 t+1의 종속변수 $r_{i,t+1}$을 예측하기 위해 $\beta_{i,t+1}$을 예측할 필요가 없습니다.

팩터 프리미엄도 예측할 필요가 없습니다. 모든 유형의 팩터 모델에서는 추정치가 일정 기간 동안 유지될 것이라고 가정하는 것이 일반적입니다. 따라서 펀더멘털 팩터모델에서는 시간 1,...T에서 추정된 팩터 프리미엄 f가 T+1에도 유효하다고 가정합니다. 그러기에 팩터 프리미엄도 예측할 필요가 없습니다.

예측은 이코노믹 팩터 모델을 사용할 때 필요합니다. 이코노믹 팩터 모델에서는 설명 변수인 팩터 프리미어 값을 예측 해야 합니다. 이코노믹 팩터 모델에서 설명 변수인 팩터 프리미엄은 GDP, CPI, 지수 수익률과 같은 값이고 이는 시간 T가 끝날 때만 알수 있습니다. 따라서 시간 T가 끝날때 우리는 T+1 시간에 대한 팩터 프리미엄 값을 알지 못하므로 $f_{T+1}$을 알지 못하면 $r_{i,T+1}$을 예측할 수 없습니다.

다만 팩터 노출도는 일정하게 유지된다고 가정하 수 있으므로 $\hat{\beta}_i$는 T+1 기간에도 여전히 유요합니다.
...
이 모델의 예측 능력을 검증하려면 간단한 테스트가 필요합니다. 먼저, 모델이 지난 데이터를 통해 어떤 관계를 보이는지 추정합니다. 만약 b의 추정치가 통계적으로 유의미하다면, 이 모델은 경제 팩터 모델을 활용한 시장 팩터의 예측에 사용될 수 있습니다. 경제 팩터 모델에 추가적인 팩터이 포함되어 있다면, 이와 유사한 회귀 분석을 통해 다른 요인의 예측값도 얻을 수 있습니다. T월의 선행 경제 지표를 바탕으로, 이 모델을 사용해 T+1 시점의 팩터 프리미엄을 예측하는 것이 가능합니다.

여기서 포인트는 회귀 분석을 이용해 팩터의 값을 예측한다는 것입니다. T월의 경기 선행지표(LI)가 주어지면 앞의 모델에서 결정된 수익률과 추정 분산을 사용하여 T+1의 팩터 프리미엄 (S&P500 수익률)을 예측하고 이를 주식 모델링에 사용할 수 있습니다.
...
지금까지 시간 T+1에 대한 팩터 프리미엄의 분포 즉 $f_{T+1}$을 예측하는 방법을 알아봤습니다. 앞서 언급 했듯이 T+1 시점의 팩터 노출도인 $\beta_{i,T+1}$은 관측 가능하거나(펀더멘털 팩터모델), T시점까지 주정된 팩터 노출도와 동일(이코노믹 팩터 모델)하다고 가정합니다.
...
가중치를 부여하는 것입니다. 복잡한 가중치 할당 방법은 현대 포트폴리오 이론의 아이디어를 바탕으로 포트폴리오의 기대 수익률을 극대화하고 전체 위험을 최소화하는 방식으로 주식에 가중치를 부여하는 것입니다. 벤치마크를 기준으로 포트폴리오를 관리하는 대부분의 포트폴리오 매니저의 경우 가중치를 결정할 때 목표는 다양합니다. 예를 들어, 매니저는 종목의 가중 평균 팩터 노출이 벤치마크의 가중 평균 팩터 노출과 같도록 종목에 가중치를 부여할 수 있습니다. 아니면  벤치마크 대비 포트폴리오의 허용 추적 오차에 제한을 두면서 초과 기대 수익을 극대화할 수 있도록 가중치를 할당 할 수도 있습니다.

포트폴리오 이론에 따른 비중 결정은 기대 수익률을 극대화 하고 위험을 최소화 하는 것입니다. 이를 위해 매니저는 각 주식의 기대 수익률, 각 주식의 분산, 주식들 간의 공분산을 알아야 합니다. 또한 매니저는 포트폴리오를 만들때 특정 제약을 따르도록 비중을 결정할 수도 있습니다. 예를 들어 공매도를 금지하거나, 각 주식에 할당된 비중에 범위를 지정할 수도 있습니다. 이후 매니저는 매개변수와 제약 조건을 고려한 이차 최적화 알고리즘을 통해 포트폴리오의 최적 가중치를 찾습니다.

이번 파트에서는 포트폴리오 가중치를 결정하는 수학적, 통계적 접근법에 대해 설명합니다.

상용 소프트웨어를 사용하는 큰 회사의 포트폴리오 관리자들에게는 선호하는 주식과 프로그램의 위험 모델의 메커니즘을 이해하는 데 도움이 될 것입니다. 자체 위험 모델과 포트폴리오 최적화 도구를 구축하고자 하는 퀀트에게는 이 파트를 통해 대략적인 방법을 알게 될것읍니다.

일단 크게 두 파트로 나눠 벤치마크가 있는 포트폴리오와 없는 포트폴리오의 비중결정 방법을 살펴봅니다.

**벤치마크가 없는 포트폴리오 비중 결정**
- ad-hoc 방법 : 다양한 경험 법칙을 사용하여 주식에 가중치를 부여합니다.
- 평균 분산 최적화 방법 : 주어진 기대 수익률에 대해 포트폴리오의 전체 위험을 최소화 하게 비중을 할당합니다.
...
시가총액 가중치는 주식의 시가총액에 비례하여 가중치를 할당하는 것입니다. 시가 총액 가중치에는 몇가지 변형 버전이 있습니다. 일부 매니저는 시가총액 제곱근으로 주식에 가중치를 부여하기도 합니다.

그 외에는 체계화 할수 없는 매니저가 자의로 비중을 할당하는 방법이 있습니다.

Ad-hoc 방식은 사실 퀀트 모델이라고 말할 수 없기 떄문에 간단히만 짚고 넘어가지만 모든 투자자들이 가장 보편적으로 쓰고 있는 방법입니다.
...
제약조건:
1. 가중치의 합은 $\sum_{i=1}^{N} \omega_i = 1$
2. 포트폴리오의 수익률은 주식의 기대 수익률과 해당 주식의 가중치의 곱으로 결정됩니다. $E(r_P) = w' \mu$
...
$$
    \underset{w}{min}\ w' \Sigma w \quad s.t. \quad w'\mu = \mu_p \quad w'l = 1
$$
...
일단 매니저는 자신이 보유한 모든 종목의 초과수익률이 있습니다. 벤치마크 대비 리스크를 관리하면서 알파가 높은 종목을 선택하는 것이 목표입니다.계층화의 첫 번째 단계는 주식 유니버스를 겹치지 않는 J개의 그룹으로 나누어 계층화하는 것입니다. 예를 들어, 주식을 산업 버킷으로 나누어 각 계층이 다른 산업을 나타낼 수 있습니다.
계층화 샘플링의 다음 단계는 각 계층에서 대표 종목을 선택하는 것입니다. 포트폴리오 관리자는 포트폴리오에 얼마나 많은 주식을 포함할지 결정해야 합니다. 
전체 주식수를 N, 포트폴리오에 포함할 주식수를 $N_P$, 계층i에 포함된 주식수를 $N_i$라고 하면 매니저는 각 계층에서 $Nj \frac{N_P}{N}$개 종목을 선택해야 합니다.
