...
펀더멘털 팩터 모델에서 팩터 노출도$\beta_{i,t}$는 주식이나 포트폴리오의 팩터에 대한 B/P, E/P 같은 현재의 지표입니다. 이는 이미 현재의 정보를 바탕으로 정해진 값입니다. 따라서 시간 t에서 이 값을 우리는 이미 알고 있습니다. 그리고 그 값으로 $r_{i,T+1}$을 예측합니다. 과거 수익률 회귀분석을 어떻게 했는지 기억해보세요. 오늘 시점의 B/P 같은 팩터 노출도와 이후 수익률을 회귀분석 했었습니다. 즉 우리는 t+1의 종속변수 $r_{i,t+1}$을 예측하기 위해 $\beta_{i,t+1}$을 예측할 필요가 없습니다.
